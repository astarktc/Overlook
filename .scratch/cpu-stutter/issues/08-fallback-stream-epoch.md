...
- 2026-08-19 (follow-up closed) — `VideoStreamEpochTests.testAStaleFirstFrameDeliveryIsRefusedAtReceiptByTheManager` drives the REAL `WebRTCManager` as the sink: a first-frame delivery queued pre-fallback and drained post-epoch-advance leaves the reconnect budget untouched and never reaches the watchdog, while the replacement stream's first frame resets the budget and fires the watchdog exactly once. Testing seams added to `WebRTCManager` (`armRenderPathForTesting`, `runFallbackEpochTransitionForTesting`, budget accessor, watchdog-delivery counter). **Mutation-verified:** deleting the sync `isCurrentEpoch` guard in `videoRenderDidReceiveFirstFrame` makes the test fail. Full suite green (65 tests). Ticket fully done — no remaining follow-ups.
